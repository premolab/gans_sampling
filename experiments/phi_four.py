...
from iterative_sir.sampling_utils.distributions import PhiFour
from iterative_sir.sampling_utils.ebm_sampling import ULA, MALA
from iterative_sir.sampling_utils.adaptive_mc import CISIR, Ex2MCMC, FlowMCMC, AugmentedULA
...
def main(config):
    ...
    target = PhiFour(config.a_coupling, config.b_field, config.N, dim_phys=config.dim_phys, beta=config.beta, tilt=tilt)

    dim = config.N
    
    prior_arg =  {'type': config.prior_type,
    ...
    for method_name, info in config.methods.items():
        ...
        if "flow" in info.dict.keys():
            ...
            flow = RealNVP_MLP(dim, 10, 1, hidden_dim=100, init_weight_scale=1e-6, prior_arg=prior_arg)
            x_init = construct_init(config.batch_size, dim, target, proposal).to(device)

            #burn-in
            if "burn_in_steps" in info.flow.dict.keys(): #MALA(**info.params.dict, dim=dim, beta=config.beta)
                x_init_ = mcmc(x_init, target, proposal, flow=flow, n_steps=info.flow.burn_in_steps, verbose=True)
                ...
                if "figpath" in config.dict.keys():
                    for i in range(x_init_.shape[0]):
                        plt.plot(x_init_[i, :].detach().cpu(), alpha=0.2, c='b')
            ...
            mcmc.verbose = False
            loss = MixKLLoss(target, proposal, flow, gamma=0.)
            flow_mcmc = FlowMCMC(target, proposal, flow, mcmc, batch_size=info.flow.batch_size, lr=info.flow.lr, loss=loss)
            flow.train()
            out_samples, nll = flow_mcmc.train(n_steps=info.flow.n_steps, init_points=x_init, start_optim=info.flow.start_optim, alpha=1.)
            flow.eval()
            mcmc.flow = flow
    ...
    if "figpath" in config.dict.keys():
        ...
        for ax, sample, name in zip(axs, flow_samples, names):
            for i in range(sample.shape[0]):
                ax.plot(sample[i, :].detach().cpu(), alpha=0.05, c='b', linewidth=3)
        ...
        for ax, sample, name in zip(axs, mixing_samples, names):
            for i in range(len(sample)):
                ax.plot(sample[i].detach().cpu(), alpha=0.3, c='g', linewidth=3)
            ax.set_title(fr'{name}')

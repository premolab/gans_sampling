...
from iterative_sir.sampling_utils.distributions import GaussianMixture, IndependentNormal
...
def main(config, run=True):
    ...
    if run:
        ...
        X_train, means = prepare_25gaussian_data(config.train_size,
                                                config.sigma, 
                                                config.random_seed)
        ...
        s = 1
        ###create points
        coef_gaussian = 1./num_gauss
        target_args.p_gaussians = [torch.tensor(coef_gaussian)]*target_args.num_gauss
        locs = torch.stack([torch.tensor([(i-2)*s,(j-2)*s] + [0]* (dim - 2)).to(device) 
                for i in range(n_col) for j in range(n_row)], 0)
        target_args.locs = locs
        target_args.covs = [(config.sigma**2)*torch.eye(dim).to(device)]*target_args.num_gauss
        target_args.dim = dim
        ...
        proposal = IndependentNormal(device=device, loc=loc_proposal, scale=scale_proposal)
        ...
        for method_name, info in config.methods.items():
            ...
            sample = sample[-config.n_chunks * info.every:].reshape((config.n_chunks, batch_size, -1, sample.shape[-1]))
            Xs_gen = sample.reshape(batch_size, config.n_chunks, -1, sample.shape[-1])

            evol = defaultdict(list)
            for X_gen in Xs_gen:
                evolution = Evolution(target_sample, locs=locs, 
                                target_log_prob=true_target, sigma=config.sigma, scaler=scaler)
                for chunk in X_gen:
        ...
        if 'respath' in config.dict:
            pickle.dump(evols, Path(config.respath, 'gaussians_2d_metrics.pkl').open('wb'))
    ...
    if 'figpath' in config.dict:
        ...
        plot_chain_metrics(evols, colors=colors, every=info.every, savepath=Path(config.figpath, 'gaussians_2d.pdf'))

...
def mix_kl(
    ...
    est_f = forward_kl(target, proposal, flow, y)
    est_b = backward_kl(target, proposal, flow, y)
    if torch.isnan(est_b).item():
        est_b = 0

    loss = alpha * est_f + (1.0 - alpha) * est_b
    if beta:
        loss -= beta * entropy(target, proposal, flow, y, acc_rate=acc_rate)
    if gamma:
        loss += gamma * importance_forward_kl(target, proposal, flow, y)
    return loss
